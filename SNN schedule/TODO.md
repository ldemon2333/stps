...
当前表格最大的问题是：`tab:q0_main` 同时放了 card-CV/JFI、max/min、throughput、
congestion、p99，caption 又把 STPS 写得像“全面负载均衡更好”。但表内数据并不支持
这个强口径：STPS 在 card-CV/JFI 上通常不是最优，真正稳定的优势是 NoC congestion，
...
> 控制 worst-card tail skew，但不保证 card-CV/JFI 这类平均离散度指标处处最优。

因此所有表格都要按三类指标分开解释：

- **NoC 效果**：congestion ratio、cong. wait。这里应是 STPS 的主胜利点。
- **负载均衡效果**：card-CV/JFI/LIF/max-min。这里要区分 average dispersion 与
...
**当前问题**：主表 caption 说 STPS 在 16 cards 上控制 max/min，但 card-CV/JFI 多数
...
- 指标分组：
  ...
  - balance：建议保留 card-CV/JFI，再增加或替换为 `LIF = max/mean`，弱化 max/min；
  - cost：throughput、p99。
...
- STPS 不应被描述为 average load balance 最优；card-CV/JFI 允许不是最优。
...
- 正文增加一句：`card-CV/JFI measure average dispersion; max/min or LIF measures tail risk.`
...
**当前问题**：表里 baseline + phase-shift 的 congestion 几乎清零，但 card-CV 暴涨；
...
- 指标分组：
  ...
  - Stage A 效果：card-CV/JFI/LIF 或 max/min；
  - full tradeoff：congestion 下降但不把 card-CV 推到 baseline+ps 那么高。

**预期效果**：

- Stage B 对所有 spatial policy 都能显著降低 congestion。
- 单独 Stage B 可能恶化 card-CV，这是 de-synchronization 在 binding cap 下的指标反转。
- Stage A 单独应保持或改善 spatial distribution，但不能解决 congestion。
- Full STPS 不追求单列最优，而是在 congestion、card-CV、p99、throughput 之间形成 deployable compromise。

**表格/文字改法**：

- Caption 不要说 full STPS congestion drop 小是“Stage A 吸收了 slack”，这个解释容易被质疑。
  更稳的说法：baseline+ps 是激进 temporal smoothing，代价是 card-CV/p99/throughput；full STPS 是受控折中。
...
- 指标：card-CV 为主；最好补 JFI 或 LIF 作为稳健性验证。
...
- 指标：card-CV；建议补 LIF 或 max/min，避免只靠平均离散度。
...
- 指标：cong. ratio、cong. wait、card-CV、LIF/max-min、throughput、p99、mean offset。
...
**当前作用**：定义 A/B/C 三个 regime，让后面 card-CV 反转不显得偶然。
...
- 指标：baseline congestion、baseline card-CV、baseline p99、offset-infeasible rate。

**预期效果**：

- A：cap 本身已经 clipping peaks，served-load 看起来较平，但 queue 很重。
- B：没有 queue，card-CV 反映真实 burst 分布。
...
### A.8 `tab:q2_regime_cv`：解释 card-CV 为什么会反转，保护 Table 1 的负载均衡叙事

**当前作用**：证明 card-CV 的方向由 cap-binding regime 决定，不是 STPS 特有失败。
...
- 到达模式：Poisson、Bursty。
- 三个 regime：A/B/C。
- 指标：`Delta card-CV = (off - on) / off`，正值表示 phase-shift 改善 card-CV。

**预期效果**：

- A 中所有 policy 都为负：phase-shift 提高 card-CV。
- B/C 中多数或所有 policy 为正：phase-shift 降低 card-CV。
- 这说明 Table 1 的 card-CV 不优不是 STPS 独有问题，而是 binding cap 下 served-load metric 的定义问题。

**表格/文字改法**：

- 正文要把它和 Table 1 连起来：Table 1 所在 cap 是 binding，因此 card-CV 不是唯一 load-balance 读数。
- 不要用该表声称 STPS 在 A 下改善负载均衡；应该说 A 下没有 scheduler 改善 card-CV。

**成功判定**：读者不会再用 Table 1 的 card-CV/JFI 直接否定 STPS，而会理解指标和 regime 的关系。
...
- 三个 regime 下，phase-shift 对 card-CV、JFI、LIF、max/min、congestion、throughput、p99 的方向。
- 该表应作为 `tab:q2_regime_cv` 的 companion，而不是单独支撑结论。

**预期效果**：

- congestion：A/C 下降，B 无变化。
- card-CV/JFI/LIF：A 方向变差，B/C 方向变好。
...
- [ ] 在 Metrics 段明确区分：
      `card-CV/JFI = average dispersion`，`LIF/max-min = tail skew`。
