...
\midrule
\multirow{8}{*}{Poisson}
 & RR \textsc{base}             & 0.676 & 0.767 & 0.101 & 3.34 & 31.0 & 0.00 \\
 & RR \textsc{+ps}              & 0.578 & 0.593 & \textbf{0.000} & \textbf{0.00} & \textbf{15.0} & 0.43 \\
 & BestFit \textsc{base}        & 0.647 & 0.767 & 0.082 & 2.44 & 24.0 & 0.00 \\
 & BestFit \textsc{+ps}         & \textbf{0.574} & 0.638 & 0.000 & 0.00 & 15.6 & 0.46 \\
 & P2C \textsc{base}            & 0.717 & 0.766 & 0.098 & 3.52 & 29.0 & 0.00 \\
 & P2C \textsc{+ps}             & 0.832 & 0.610 & 0.000 & 0.00 & 16.8 & 0.63 \\
 & \textsc{stps-spatial}        & 0.659 & 0.767 & 0.084 & 2.48 & 25.0 & 0.00 \\
 & \textbf{stps-spatial+ps}     & 0.615 & 0.767 & 0.087 & 2.62 & 25.2 & 0.66 \\
\midrule
\multirow{8}{*}{Bursty}
 & RR \textsc{base}             & 0.707 & 0.758 & 0.103 & 3.98 & 32.8 & 0.00 \\
 & RR \textsc{+ps}              & 0.665 & 0.586 & \textbf{0.000} & \textbf{0.00} & \textbf{16.2} & 0.52 \\
 & BestFit \textsc{base}        & 0.683 & 0.763 & 0.087 & 3.18 & 30.2 & 0.00 \\
 & BestFit \textsc{+ps}         & 0.678 & 0.615 & 0.000 & 0.00 & 16.5 & 0.51 \\
 & P2C \textsc{base}            & 0.735 & 0.761 & 0.101 & 4.24 & 31.2 & 0.00 \\
 & P2C \textsc{+ps}             & 0.875 & 0.588 & 0.000 & 0.00 & 17.0 & 0.64 \\
 & \textsc{stps-spatial}        & 0.691 & 0.762 & 0.088 & 3.19 & 28.6 & 0.00 \\
 & \textbf{stps-spatial+ps}     & \textbf{0.662} & 0.760 & 0.088 & 3.17 & 29.8 & 0.66 \\
\bottomrule
\end{tabular}
\end{table*}

\paragraph{Phase-shift zeroes the mild congestion at $D_{\max}\!=\!2$.} Table~\ref{tab:q2_phase} reports the ablation. At this lightly-congested workpoint the baseline congestion ratio is already modest ($\sim$0.08--0.10, queue wait 2.4--4.2 ticks); attaching phase-shift to RR, BestFit, DRF, or P2C drives it to exactly zero and empties the queue. The mechanism is direct: by deferring each task's onset by at most $D_{\max}\!=\!2$ ticks the optimiser finds a slot whose superposition on the card forecast stays below $BW_{\max}$, so p99 falls with it (e.g.\ RR Poisson $31.0\!\to\!15.0$). The cost is throughput: mean offsets stay near half a tick, but the cap still rejects the tasks that cannot find a sub-cap slot, and the steady-state window clips the deferred onsets, so throughput drops 17--23\,\% (RR Poisson $0.767\!\to\!0.593$). \textsc{stps-spatial+ps} (full STPS) is the exception that proves the point: it holds the congestion ratio at $\sim$0.087 --- essentially unchanged from \textsc{stps-spatial}'s 0.084 --- because Stage~A has already spread the load spatially, leaving almost nothing for Stage~B to drain, and in return it pays \emph{no} throughput (0.767, identical to its base) since its hotspot-split admission never triggers a cap rejection.

\paragraph{Phase-shift lowers CV when the cap does not clip.} The central observation in Table~\ref{tab:q2_phase} is that, unlike the cap-saturated regime, phase-shift here \emph{lowers} CV for RR (Poisson $0.676\!\to\!0.578$), BestFit ($0.647\!\to\!0.574$), and full STPS ($0.659\!\to\!0.615$). The mechanism is mechanical and is exactly the sign-flip mapped in Appendix~\ref{app:regime}: CV measures the across-card variance of instantaneous served load, and when the cap does \emph{not} clip, de-synchronising bursts that previously peaked together lowers each card's peak-to-mean ratio and so reduces across-card dispersion directly. This is the regime an operator should target, and it confirms that the CV inflation seen under a hard-binding cap is an artefact of the cap flattening the baselines to an artificially low $\approx$0.27, not an intrinsic cost of phase-shift. The lone exception is P2C ($0.717\!\to\!0.832$, $0.735\!\to\!0.875$): its power-of-two-choices base already de-correlates cards by instantaneous load, so adding temporal de-synchronisation over-disperses an already-spread distribution. The sign of phase-shift's CV effect is thus set by whether the cap binds and by how much spatial structure the base policy has already imposed, not by phase-shift in isolation.

\paragraph{The two stages occupy complementary roles.} The two stages cover complementary failure modes, and the lightly-congested workpoint makes the division of labour legible. Phase-shift on a capacity baseline drives congestion to exactly zero but at a steep 17--23\,\% throughput cost, and for P2C it inflates CV; macro-dispatch alone (\textsc{stps-spatial}) holds CV and throughput at the baseline but leaves the mild congestion ($\sim$0.084) untouched. Only the pairing reaches a deployable compromise: full STPS posts the lowest CV among the STPS variants ($0.615$/$0.662$), keeps congestion mild ($\sim$0.087), and --- crucially --- loses \emph{no} throughput, whereas every baseline+phase-shift row that zeroes congestion sacrifices nearly a fifth of it. Stage~A primarily shapes the across-card distribution and protects throughput; Stage~B reshapes each card's timeline and, in this regime, pushes CV down rather than up. Their interaction prevents one stage from reintroducing the failure mode handled by the other. Table~\ref{tab:q2_phase} should therefore be interpreted by considering each row's full metric vector --- congestion, CV, tail delay, and throughput together --- not a single column: a policy that zeroes one number by giving up nearly a fifth of throughput is not the operating point STPS targets.
...
\begin{table}[t]
\centering
\caption{Cross-regime $\Delta$CV under \textsc{+ps} (5 seeds, $(off\!-\!on)/off \times 100$; positive $=$ phase-shift reduces CV). Every cell of the binding regime is negative; every cell of the non-binding and light regimes is positive. The direction of phase-shift's effect on CV is determined entirely by whether the cap is binding, independent of arrival mode, baseline policy, or even the spatial policy that phase-shift is attached to.}
...
\midrule
CV          & $\uparrow$ & $\downarrow$ & $\downarrow$ \\
