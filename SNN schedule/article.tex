...
\begin{table*}[!t]
\centering
\caption{Stage decomposition at the default congested workpoint (16 cards, mixed fingerprints, $BW_{\max}\!=\!9\!\times\!10^{5}$, $D_{\max}\!=\!2$, 1024 ticks, 5 seeds): each spatial policy (\textsc{base}) paired with phase-shift (\textsc{+ps}), and STPS's own macro-dispatch \textsc{stps-spatial} with phase-shift added; the full scheduler is \textsc{stps-spatial+ps}. Attaching phase-shift to a capacity baseline drives the congestion ratio to zero, but the price is steep: throughput drops 28--34\,\% because tasks that cannot find a sub-cap offset are held back. Full STPS is the exception: it lowers congestion at essentially no throughput cost, because Stage~A has already spread the load and Stage~B has little left to defer or to reject. Read each row as a full metric vector, not a single column.}
\label{tab:q2_phase}
\footnotesize
...
\multirow{8}{*}{Poisson}
 ...
 & RR \textsc{+ps}              & 4.494 & \textbf{0.000} & \textbf{0.00} & 1.05 \\
 & BestFit \textsc{base}        & \textbf{6.074} & 0.107 & 1.03 & 0.00 \\
 & BestFit \textsc{+ps}         & 4.849 & 0.000 & 0.00 & 0.98 \\
 & P2C \textsc{base}            & \textbf{6.074} & 0.128 & 1.48 & 0.00 \\
 & P2C \textsc{+ps}             & 4.280 & 0.000 & 0.00 & 1.03 \\
 & \textsc{stps-spatial}        & \textbf{6.074} & 0.112 & 1.09 & 0.00 \\
 & \textbf{stps-spatial+ps}     & 6.056 & 0.097 & 0.85 & 1.15 \\
\midrule
\multirow{8}{*}{Bursty}
 & RR \textsc{base}             & 6.074 & 0.129 & 1.58 & 0.00 \\
 & RR \textsc{+ps}              & 4.241 & \textbf{0.000} & \textbf{0.00} & 0.95 \\
 & BestFit \textsc{base}        & 6.074 & 0.120 & 1.39 & 0.00 \\
 & BestFit \textsc{+ps}         & 4.472 & 0.000 & 0.00 & 0.89 \\
 & P2C \textsc{base}            & 6.074 & 0.139 & 1.84 & 0.00 \\
 & P2C \textsc{+ps}             & 4.013 & 0.000 & 0.00 & 0.98 \\
 & \textsc{stps-spatial}        & 6.074 & 0.127 & 1.53 & 0.00 \\
 & \textbf{stps-spatial+ps}     & 6.054 & 0.106 & 1.20 & 1.07 \\
\bottomrule
\end{tabular}
\end{table*}
...
\begin{table}[t]
\centering
\caption{How each metric responds as the phase-shift budget $D_{\max}$ grows from 2 to 16 (light regime C, 16 cards, mixed fingerprints, 5 seeds). Congestion --- the metric STPS targets --- is a switch, not a knob: it is already zeroed at $D_{\max}\!=\!2$. The dispersion diagnostics (CV/JFI/LIF) do improve with a larger budget in this loosely-capped regime, but with diminishing returns that saturate near $D_{\max}\!=\!8$, while throughput cost grows monotonically. The columns pull in opposite directions, which is what fixes a small default.}
\label{tab:q2_dmax}
\small
\begin{tabular}{lll}
\toprule
...
Mean offset              & $\uparrow$ monotone & $0.5\!\to\!4$ ticks \\
